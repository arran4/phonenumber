...
import (
	...
	"phonenumber"
	"phonenumber/cmd"
	"phonenumber/cmd/drawphonecli/templates"
)
...
func NewRoot(name, version, commit, date string) (*RootCmd, error) {
	...
	c.StringVar(&c.outfile, "outfile", "out.png", "The output filename")

	c.StringVar(&c.text, "text", "Hello how are you?", "Text to write")

	c.Float64Var(&c.fontsize, "fontsize", 12, "Font size")

	c.CommandAction = func(c *RootCmd) error {

		err := phonenumber.RunCLI(c.outfile, c.text, c.fontsize)
		if err != nil {
			...
			return fmt.Errorf("drawphonecli failed: %w", err)
		}
		return nil
	}
